...
const CompanyEmpTableWrapper = () => {
    ...
    const fullColumns = useMemo(() => {
        return [
            ...
            {
                field: "fixedSalaryAmount", headerName: "Fixed Salary Amount", align: "left", render: (row) => (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Typography>{row.fixedSalaryAmount}</Typography>
                    <CurrencyType />
                </Box>)
            },
            { field: "actualJoiningDate", headerName: "Actual joining date", align: "left" },
            { field: "c3_card", headerName: "C3 Card", align: "left" },
            { field: "c3_card_date", headerName: "C3 Card Applied Date", align: "left" },
            { field: "status", headerName: "Status", align: "left" },
            { field: "employeeType", headerName: "Employee Type", align: "left", render: (row) => (<Typography sx={{ px: 1.5, borderRadius: 1, bgcolor: "#2F2B3D14", width: "fit-content" }}>{row.employeeType}</Typography>) },
